...
export const getGroupPageData = async (): Promise<ServicesGroupPageProps> => {
  ...
  const dataArticle: ArticleProps = {
    content: <ul className="list-disc ml-6 space-y-2">
      <li>Группы формируются в зависимости от уровня подготовки участников.</li>
      <li>Набор в группы проводится ежегодно в сентябре.</li>
      <li>Для записи в уже сформированную группу в течение года у Вас должен быть минимальный опыт верховой езды, Вы должны уметь самостоятельно чистить лошадь (пони), седлать и выводить на манеж. Этому можно обучиться на индивидуальных занятиях.</li>
    </ul>,
